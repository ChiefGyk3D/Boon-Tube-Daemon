...
shared with Typo Sniper, Stream Daemon and Star Daemon, so a pipeline fix or a
new scan step lands once. Each file here says only what is specific to Boon
Tube Daemon: Python versions, the test command, the Dockerfile path, the
Doppler project.
...
The arm64 leg compiles SQLite from source (CVE-2025-7709) under QEMU and
installs every dependency twice, so `release.yml` allows 90 minutes.
...
Before that is set the pipelines still run: Codecov upload skips with a
notice, Snyk warns and skips, GHCR publishing works regardless because it uses
...
```sh
cosign verify ghcr.io/chiefgyk3d/boon-tube-daemon:latest \
  --certificate-identity-regexp '^https://github.com/ChiefGyk3D/git-your-ship-together/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

gh attestation verify oci://ghcr.io/chiefgyk3d/boon-tube-daemon:latest --owner ChiefGyk3D
```
...
(`docker/Dockerfile`) and GitHub Actions.

## Status badges

```markdown
[![CI](https://github.com/ChiefGyk3D/Boon-Tube-Daemon/actions/workflows/ci.yml/badge.svg)](https://github.com/ChiefGyk3D/Boon-Tube-Daemon/actions/workflows/ci.yml)
[![Release](https://github.com/ChiefGyk3D/Boon-Tube-Daemon/actions/workflows/release.yml/badge.svg)](https://github.com/ChiefGyk3D/Boon-Tube-Daemon/actions/workflows/release.yml)
[![Security](https://github.com/ChiefGyk3D/Boon-Tube-Daemon/actions/workflows/security.yml/badge.svg)](https://github.com/ChiefGyk3D/Boon-Tube-Daemon/actions/workflows/security.yml)
```
...
- `dependabot.yml` watched `/Docker`, a directory this repository does not
  have; it now watches `/docker`.
