...
"""
Test script for posting to all configured platforms.
Tests Discord, Matrix, Bluesky, and Mastodon with their configured styles.
"""
...
def main():
    print("\n" + "="*70)
    print("🚀 Multi-Platform Posting Test with Configured Styles")
    print("="*70)
    ...
    # Discord
    if os.getenv('DISCORD_ENABLE') == 'true' or os.getenv('DISCORD_ENABLE_POSTING') == 'true':
    ...
    # Matrix
    if os.getenv('MATRIX_ENABLE') == 'true' or os.getenv('MATRIX_ENABLE_POSTING') == 'true':
        print("\n🟪 Initializing Matrix...")
    ...
    # Bluesky
    if os.getenv('BLUESKY_ENABLE') == 'true' or os.getenv('BLUESKY_ENABLE_POSTING') == 'true':
    ...
    # Mastodon
    if os.getenv('MASTODON_ENABLE') == 'true' or os.getenv('MASTODON_ENABLE_POSTING') == 'true':
    ...
    for platform_name, platform in platforms.items():
        ...
        # Confirm posting
        response = input(f"\n✋ Post to {platform_name.upper()}? (y/N): ").strip().lower()
        if response != 'y':
            print(f"⏭ Skipped {platform_name}")
            results[platform_name] = 'skipped'
            continue
